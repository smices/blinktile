...
const engine = fresh();
assert(lit(engine.frame(0)), 'boot must show a pet smile');
assert.deepEqual(engine.frame(0),engine.frame(1000),'quiet smile must remain still');
const wink=fresh();
const quiet=wink.frame(0);
const open=wink.frame(26000);
assert.deepEqual(open,wink.frame(28700),'wink starts with the same open face');
const closed=wink.frame(28850);
assert.notDeepEqual(closed,open,'wink closes one eye once');
ok(wink,{id:90,op:'idle',mode:'pet'},28850);
assert.deepEqual(wink.frame(28850),closed,'repeated idle pet must not restart a wink');
assert.deepEqual(wink.frame(29000),quiet,'wink returns to quiet smile');
const heart=fresh();
const heartQuiet=heart.frame(0);
assert.notDeepEqual(heart.frame(53000),heartQuiet,'heart appears after a quiet dwell');
assert.deepEqual(heart.frame(54200),heartQuiet,'heart ends after one authored cycle');
