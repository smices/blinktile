...
assert(!lit(engine.frame(1001)), 'TTL expires while paused');
...
assert(!lit(engine.frame(2501)), 'renewed TTL still expires');
...
assert(!lit(text.frame(10000)), 'one-pass text releases');
