...
(async()=>{
  ...
  client.disconnect();
  const pairing=new Client();pairing.mode='ble';pairing._ready=true;
  let attempts=0;
  pairing.send=async command=>{assert.equal(command.op,'auth');assert(!('token' in command));return ++attempts===1?{ok:false,error:'unauthorized'}:{ok:true};};
  await pairing._auth('',pairing._generation);
  assert.equal(attempts,2,'BLE button pairing should retry without exposing a token');
  console.log('PASS client: auth isolation, clock sync, cancellation, simulator label, BLE chunk isolation, button pairing');
