 ...
 while(std::getline(std::cin,line)){
  ...
  if(!petTimes.isNull()){
   ...
   for(JsonVariantConst sample:petTimes){
    ...
    for(int i=0;i<64;i++){hash=(hash^frame[i])*16777619u;lit+=frame[i]>0;invalidColumn+=frame[i]>0&&i%8!=0&&i%8!=1&&i%8!=2&&i%8!=4&&i%8!=5&&i%8!=7;}
...
def main():
    ...
    try:
        ...
        for sample in samples:
            if sample['mood']==0:
                assert sample['icon']=='' and sample['lit']<=30 and sample['invalid_column']==0, 'matrix rain must use six short vertical lanes'
            elif sample['mood']==1:
        ...
        assert max(sample['lit'] for sample in rain)-min(sample['lit'] for sample in rain)>=8, 'rain density must vary'
