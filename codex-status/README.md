...
同一端口只能运行一个桥接实例。若已有桥接在 `127.0.0.1:8766` 运行，无需重复启动；要重启或切换通道，先在运行它的终端按 `Ctrl+C` 停止。macOS 首次使用时，还需允许运行 Python 的终端访问蓝牙。

```bash
.venv/bin/python -m pip install bleak
read -rsp 'control_token: ' ICONSHOW_TOKEN; echo
