...
BLE 广播名和设备热点前缀均为 `BlinkTile`。源码目录和发布包名 `IconShow-release.zip` 暂沿用旧标识；下文命令与当前固件、构建脚本保持一致。
...
在支持的桌面 Chrome/Edge 打开 `http://127.0.0.1:8000`。选图标或输入 `100%`，调整参数，再点击发送。WebSocket 地址为 `ws://设备IP:81/ws`。网页控制只是可选入口：当前固件支持 BLE 留空控制密钥后按住设备 BOOT 完成本次连接授权，但这条按键路径尚未真机验收。网页 BLE 路径也尚未完成真机验收。[Codex 状态桥接](codex-status/README.md)已使用现有控制密钥通过 BLE 真机连接，不需要本地 HTTP 服务或浏览器。完整连接说明见[设备配置与验收](docs/hardware.md)。
...
不需要电脑、串口、家用 Wi-Fi 或互联网。首次上电时，灯板会滚动显示 `PASS XXXXXXXX`；在手机 Wi-Fi 设置中连接同后缀的 `BlinkTile-XXXX`，输入这 8 位设备密码，保留连接（即使手机提示没有互联网），然后打开 `http://192.168.4.1`。设备内置页面可直接显示时间、切换桌宠、关灯、调亮度，也可选择性配置 2.4GHz Wi-Fi。页面资源和控制链均在设备本地。

热点在配网成功后继续运行，直到用户在页面中明确点击“完成并关闭热点”。以后长按设备运行中的 BOOT 约 5 秒，可以重新开启热点并重显密码；不会清除已有 Wi-Fi。先试连、取得 IP 才保存，失败时保留旧配置和热点。USB 串口仍是开发和恢复入口，但不是普通用户首次使用的前提。

BLE 和局域网 WebSocket 仍供 Codex 状态桥接及其他应用使用，不因手机直连入口而删除。
