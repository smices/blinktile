...
桌宠以暖黄色慢速微笑为安静基态，稳定 10～15 秒后随机做一次嫩绿开心、橙色眨眼、蓝色困倦、粉色惊讶或红粉色心跳，再回到微笑；特殊行为维持约 5～10 秒，动作完整播放后才切换。启动默认进入桌宠；取得有效时间后先以 24 小时制滚动显示 `HH:MM`，此后每隔随机 2～7 分钟显示一次，不显示时区和秒。软件状态临时覆盖，到期或释放后恢复。
