...
桌宠上电先显示绿色字符雨，持续随机 36～48 秒；随后显示暖黄色微笑 4 秒，再随机播放一次眨眼或心形动作约 2.2 秒，然后返回字符雨。独立图鉴仍保留开心、惊讶和困倦图标，但桌宠不自动使用它们。自动报时首次在开机后随机 2～7 分钟触发，之后每隔随机 2～7 分钟滚动一次 24 小时制 `HH:MM`，不显示时区和秒；主动发送 `time` 可随时显示，手动从 off 切换到 pet 时优先报时。软件状态临时覆盖，到期或释放后恢复字符雨。网页 GIF 是代表性节奏示意，真机的字符和动作选择、间隔为随机值。
