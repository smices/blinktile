...
普通用户首次使用不依赖上述 token 或串口：设备无已保存网络时开启 `BlinkTile-XXXX` WPA2 热点，并在灯板滚动显示 8 位密码。手机连接后访问 `http://192.168.4.1`；AP-only 页面通过已有 WebSocket 认证在设备内部完成基本控制和可选配网。热点只在用户明确结束时关闭，运行中长按 BOOT 约 5 秒可重开并重显密码。
