...
桌宠以暖黄色慢速微笑为主，随机穿插快速嫩绿开心、短促橙色眨眼、慢速蓝色困倦、粉色惊讶，以及红粉色心形连续跳动 2～3 次；动作完整播放后才切换，并避免连续重复。启动默认进入桌宠；取得有效时间后先以 24 小时制滚动显示 `HH:MM`，此后每隔随机 2～7 分钟显示一次，不显示时区和秒。软件状态临时覆盖，到期或释放后恢复。
