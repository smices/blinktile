...
桌宠上电固定从暖黄色慢速微笑开始，稳定 10～15 秒后，以心跳为主要动作，穿插眨眼和低频困倦，再回到微笑；不使用张嘴的开心和惊讶表情。特殊行为维持约 5～10 秒，动作完整播放后才切换。取得有效时间后先以 24 小时制滚动显示 `HH:MM`，此后每隔随机 2～7 分钟显示一次，不显示时区和秒。软件状态临时覆盖，到期或释放后恢复。
