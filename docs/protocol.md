...
每个请求的 `id` 为 `1～2147483647` 整数。最大请求 512 个 UTF-8 字节；BLE 末尾追加换行，不计入 JSON 长度，每次最多写入 20 字节并顺序等待完成。BLE 响应同样换行分帧，可以跨多个通知；响应接收上限为 2048 字节。断线丢弃未完成帧，组包超时 2 秒。
...
桌宠上电先显示暖黄色睁眼微笑，安静保持随机 20～45 秒；之后以约 4:1 的概率做一次单眼眨眼或一次心形跳动，随即回到微笑。眨眼沿用 3 秒素材周期，其中闭眼约 200ms；心形只播放一次约 1.2 秒。独立图鉴仍保留开心、惊讶和困倦图标，但桌宠不自动使用它们。自动报时首次在开机后随机 2～7 分钟触发，之后每隔随机 2～7 分钟滚动一次 24 小时制 `HH:MM`，不显示时区和秒；主动发送 `time` 可随时显示，手动从 off 切换到 pet 时优先报时。软件状态临时覆盖，到期或释放后恢复微笑。网页 GIF 是代表性节奏示意，真机的动作选择与间隔为随机值。
